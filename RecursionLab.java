...
public class RecursionLab {
    ...
    public static int cutRecExponent(int x, int n) {
        if (n == 0) {
            return x;
        }

        if (n / 2 % 2 == 0) {
            return cutRecExponent(x, n / 2) * cutRecExponent(x, n / 2);
        } else {
            return x * cutRecExponent(x, (n - 1) / 2) * cutRecExponent(x, (n - 1) / 2);
        }
    }
    ...
    public static int nChooseR(int n, int r){
        ...
        int nFactorial = recursiveFactorial(n);
        int rFactorial = recursiveFactorial(r);
        int nMinusR = recursiveFactorial(Math.abs(r - n));
